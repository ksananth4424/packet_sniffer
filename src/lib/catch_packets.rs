use crate::lib::parse_packets::{HeaderPacket, ParsePacket, PacketParsed};
...
impl CatchPackets {
    ...
    pub fn save_to_file(&mut self, mut cap_handle: Capture<Active>, file_name: &str) {
        match cap_handle.savefile(file_name) {
            ...
            Err(err) => self.print_err(err.to_string()),
        }
    }
    ...
    pub fn print_to_console(&mut self, mut cap_handle: Capture<Active>) {
        ...
        while let Ok(packet) = cap_handle.next() {
            ...
            let packet_parse = ParsePacket::new();
    ...
    fn get_packet_meta(&self, parsed_packet: &PacketParsed) -> (String, String, String, String) {
    ...
    fn print_packet(&self, parsed_packet: &PacketParsed) {
