...
fn is_i32(val: String) -> Result<(), String> {
    match val.parse::<i32>() {
        Ok(_) => Ok(()),
        Err(err) => Err("Invalid value".to_string()),
    }
}
...
fn is_precision_type(val: String) -> Result<(), String> {
    ...
    if domain_set.contains(&&val[..]) {
        ...
        Err(format!("The value must be one of the following: {:?}", domain_set))
    }
}
...
impl<'a, 'b> CaptureSubcommand {
    ...
    pub fn get_subcommand(&self) -> App<'a, 'b> {
        let run_args = vec![
            ...
            Arg::with_name("timeout")
                .help("Set the read timeout for the catch. By default, this is 0, which it will block indefinitely.")
                .takes_value(true)
                .short("t")
                .long("timeout")
                .validator(is_i32),
    ...
    pub fn run_args(
        ...
        if let Some(temp) = args.value_of("precision") {
            device = device.precision(self.get_precision_type(temp).unwrap());
        }
